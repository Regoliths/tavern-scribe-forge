...
export function CharacterCreator() {
  ...
  const calculateHitPoints = (characterClass: string, level: number, constitution: number) => {
    const classHitDice = getClassHitDice();
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        {dndDataLoading ? (
          <div className="text-center text-xl text-parchment">Loading D&D data...</div>
        ) : dndDataError ? (
          <div className="text-center text-xl text-red-400">Error: {dndDataError}</div>
        ) : (
          <>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {character.class && (
                          <Badge variant="outline" className="border-gold text-gold text-xs">
                            d{getClassHitDice()[character.class]} + CON
                          </Badge>
                        )}
                      </div>
                      <div className="flex items-center space-x-2">
                      ...
                      {character.class && (
                        <p className="text-xs text-muted-foreground mt-1">
                          Level 1: {getClassHitDice()[character.class]} + {getModifier(character.constitution)} = {getClassHitDice()[character.class] + getModifier(character.constitution)} HP
                        </p>
                      )}
                    </div>
